...
// ─────────────────────────────────────────────────────────────
// REVIEW TOAST — muncul otomatis untuk tiket "Digunakan"
//   yang belum direview. Mengirim rating ke:
...
function ReviewToast({ order, onClose, onSubmitSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // await api.post(`/wisata/${order.wisata_id}/rating`, { rating }); balikin ketika udah sso
            await api.post(`/dev/rating/${DEV_USER_ID}/${order.wisata_id}`, { rating });
            setSubmitted(true);
            // Setelah 1.8 detik, tutup dan update state parent
            setTimeout(() => {
                    ...
                    {!submitted ? (
                        <>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={doClose} style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: '1.5px solid var(--border)', background: 'white', fontSize: 13, fontWeight: 600, color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit' }}>
                                    Nanti
                                </button>
                                <button onClick={handleSubmit} disabled={rating === 0 || loading}
                                    ...
                                    {loading
                                        ? <><i className="fas fa-spinner fa-spin" style={{ fontSize: 11 }} /> Mengirim...</>
...
function ETicketModal({ order, onClose }) {
    ...
    const totalOrang = (order.jumlah_dewasa || 0) + (order.jumlah_anak || 0);

    const handleDownload = () => {
        const win = window.open('', '_blank');
        if (!win) return;
        win.document.write(`<!DOCTYPE html><html><head>
            <title>E-Ticket ${order.kode_order}</title>
            ...
            <div class="ticket">
                ...
                <div class="code"><div class="code-label">Kode Booking</div><div class="code-val">${order.kode_order}</div></div>
                ${[
    ...
    {/* Tambahkan ini — QR hanya muncul jika tiket masih Aktif */}
    {!isUsed && (
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16, marginBottom: 4 }}>
            <div style={{ padding: 12, background: 'white', border: '2px solid var(--teal-200)', borderRadius: 12 }}>
                <QRCodeSVG
                        ...
                        <div>
                            ...
                            <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                                    ...
                                    <tbody>
                                        {/* Dari tabel ticket_orders tidak ada kolom items/breakdown,
                                            jadi tampilkan dewasa & anak langsung dari kolom yang ada */}
                                        {order.jumlah_dewasa > 0 && (
                                            <tr style={{ borderTop: '1px solid var(--border)' }}>
                                                <td style={{ padding: '12px 14px', textTransform: 'capitalize', fontWeight: 500 }}>Dewasa</td>
                                        ...
                                        {order.jumlah_anak > 0 && (
                                            <tr style={{ borderTop: '1px solid var(--border)' }}>
                                                <td style={{ padding: '12px 14px', textTransform: 'capitalize', fontWeight: 500 }}>Anak</td>
...
export default function RiwayatTiket() {
    ...
    // reviewToast: order yang sedang ditampilkan toast-nya
    // Hanya muncul untuk tiket status 'Digunakan' yang belum direview
    const [reviewToast,   setReviewToast]  = useState(null);
    // Set wisata_id yang sudah di-review di sesi ini (untuk skip toast)
    const [reviewedWisataIds, setReviewedWisataIds] = useState(new Set());

    // ── Fetch tiket dari API ────────────────────────────────
    // GET /api/tiket-saya → TicketOrderController@index
    // Auth: Bearer token via axios interceptor
    // Response: { success: true, data: [ { id, kode_order, wisata_id, wisata: {...}, tanggal_kunjungan,
    //            jumlah_dewasa, jumlah_anak, total_harga, status_tiket, metode_pembayaran,
    //            created_at, sudah_direview } ] }
    useEffect(() => {
        const fetchOrders = async () => {
            setLoading(true);
            setError(null);
            try {
                // const res = await api.get('/tiket-saya'); balikin ketika udah sso
                const DEV_USER_ID = '1e23d1b3-4e2e-4d6c-aba6-2d1889cfaccb';
    ...
    // ── Tampilkan review toast otomatis ────────────────────
    // Logika: setelah data loaded, cari order pertama dengan:
    //   status_tiket = 'Digunakan' && sudah_direview = false
    //   && wisata_id belum ada di reviewedWisataIds (skip yang sudah di-toast)
    useEffect(() => {
    ...
    // Rating berhasil dikirim → update state lokal
    const handleRatingSuccess = (wisataId) => {
        // Tandai wisata ini sudah direview di sesi ini
        setReviewedWisataIds(prev => new Set([...prev, wisataId]));
        // Update flag sudah_direview di semua order yang punya wisata_id ini
        setOrders(prev => prev.map(o =>
            o.wisata_id === wisataId ? { ...o, sudah_direview: true } : o
        ));
        setReviewToast(null);
    };

    const handleReviewClose = () => {
        // Tutup toast + skip wisata ini untuk sesi ini
        if (reviewToast) {
            setReviewedWisataIds(prev => new Set([...prev, reviewToast.wisata_id]));
        }
        setReviewToast(null);
    };
    ...
    const filtered = useMemo(() => {
        ...
        if (filter !== 'semua') {
            data = data.filter(o => o.status_tiket === filter);
        }
    ...
    return (
        <div className="page-riwayat" style={{ background: 'var(--cream)', minHeight: '100vh' }}>
                    ...
                    <div className="page-toolbar">
                        <div className="page-filter-tabs">
                            {FILTER_OPTIONS.map((f) => (
                                <button key={f.value} className={`page-filter-tab${filter === f.value ? ' active' : ''}`} onClick={() => setFilter(f.value)}>
                                    {f.label}
                                </button>
                            ))}
                        </div>
            ...
            {/* Review toast — HANYA muncul jika status_tiket = 'Digunakan' && belum direview */}
            {reviewToast && (
                <ReviewToast
                    order={reviewToast}
